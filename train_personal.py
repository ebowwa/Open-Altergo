...
Designed to run both on a single local CUDA GPU and inside ``modal_app.py``.
The produced ``personalized_model.pt`` is a raw E2E state dict compatible with
``engine/local_infer.py`` and the ``LIPREAD_CKPT`` application setting.
"""
...
from datamodule.data_module import DataModule  # noqa: E402
from personalization import PersonalizationModelModule  # noqa: E402
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--finetune-strategy",
        choices=sorted(PersonalizationModelModule.VALID_STRATEGIES),
