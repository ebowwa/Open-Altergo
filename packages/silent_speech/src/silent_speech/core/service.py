...
def _find_project_root() -> Path:
    """Resolve a checkout that contains the separately managed engine."""

    configured = os.environ.get("OPEN_ALTERGO_ROOT")
    candidates = []
    if configured:
    ...
    for candidate in candidates:
        ...
        if pipeline.is_file():
            return candidate
    raise RuntimeError(
        "Could not locate cloud/engine. Run from an Open-Altergo checkout or "
        "set OPEN_ALTERGO_ROOT to the repository root."
    )
