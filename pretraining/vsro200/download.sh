...
case "$SOURCE" in
  models)
    REPOSITORY_ID="vsro200/models-vsro200"
    REPOSITORY_TYPE="model"
    ;;
  mlp)
    REPOSITORY_ID="vsro200/mlp-lrro-vsro200"
    REPOSITORY_TYPE="model"
    ;;
  dataset)
    REPOSITORY_ID="vsro200/vsro200"
    REPOSITORY_TYPE="dataset"
    ;;
  *)
    printf 'Usage: %s <models|mlp|dataset> <40-character-hugging-face-commit>\n' \
      "$0" >&2
    exit 2
    ;;
esac
...
printf 'Do not commit the downloaded directory. Review sources.json.\n'
